...
class MainController(BaseController):
    ...
    def change_res(self, event):
            res_type = self.current_controller.frame.res_combo.get()
    ...
    def update_language(self, event):
        self.current_language = self.current_controller.frame.login_combo.get()
        if self.current_controller == self.login_controller:
            ...
            self.current_controller.frame.language_label.config(text=LANGUAGE[self.current_language]["language"])
            self.current_controller.frame.res_label.config(text=LANGUAGE[self.current_language]["resolution"])
            self.current_controller.frame.logout_button.config(text=LANGUAGE[self.current_language]["logout"])
        elif self.current_controller == self.customer_controller:
